...
// Sibling of the Engine Room, for the food/fuel/body strand. Reads public.fuel_foundation.
const CATEGORY_COLORS = {
  foundation:    'rgba(232,201,140,0.7)',  // the keystone + load-bearing pieces — warm gold
  open_question: 'rgba(168,144,212,0.7)',  // parked, awaiting data — lavender
  working_style: 'rgba(110,192,191,0.7)',  // how to build with Cat — teal
  method:        'rgba(180,200,150,0.7)',  // (later) — green
  data:          'rgba(126,184,217,0.7)',  // (later) food lists / recipes / plans — blue
}

const CATEGORY_ORDER = ['foundation', 'open_question', 'working_style', 'method', 'data']

// Friendlier display names; grouping still keys off the raw category value.
...
export default function FuelFoundation({ onSettings, roomName = 'Fuel Foundation' }) {
  ...
  if (selected) {
    return (
      <>
        <Header />
        {backBtn(catLabel(selectedCategory), () => { setSelected(null); setEditing(false) })}
    ...
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 }}>
        <div style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 10, letterSpacing: '0.28em', textTransform: 'uppercase', color: 'rgba(240,227,194,0.35)' }}>
          the why beneath fuel · model before method
        </div>
        <button
      ...
      {error && (
        <div style={{ padding: '20px 0', fontFamily: '"Montserrat", serif', fontStyle: 'italic', fontSize: 14, color: 'rgba(255,160,180,0.7)' }}>
          could not load fuel foundation: {error}
        </div>
      )}
